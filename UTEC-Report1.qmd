...
# `ajustar = TRUE` anade scale_down, que encoge la tabla hasta el ancho del
# texto. Necesario en la matriz de covarianzas: 10 columnas numericas de varias
# cifras se salen del margen y LaTeX no las parte solo.
...
#| tbl-cap: "Medidas de tendencia central, dispersión y forma de las variables numéricas."
leer_tabla("tbl_descriptivos.csv") |>
  select(Variable, Media, Mediana, Moda, `Desv. Est.` = Desv_Est,
         Mín = Min, Q1, Q3, Máx = Max, RIC, `CV %` = CV_pct,
         Asimetría = Asimetria, Curtosis) |>
  mutate(across(where(is.numeric), ~ round(.x, 2))) |>
  tabla_bonita("Descriptivos de las variables numéricas.", fs = 7)
```
...
# Los nombres completos no caben como encabezado: se abrevian a 10 caracteres
# y la tabla se escala al ancho del texto.
abreviar <- function(x) substr(x, 1, 10)
leer_tabla("tbl_covarianza.csv") |>
  rename(Variable = 1) |>
  mutate(Variable = abreviar(Variable),
         across(where(is.numeric), ~ round(.x, 1))) |>
  rename_with(abreviar, -Variable) |>
  tabla_bonita("Matriz de covarianzas.", fs = 6, ajustar = TRUE)
```
